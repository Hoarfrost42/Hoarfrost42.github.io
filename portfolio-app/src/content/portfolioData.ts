...
export const portfolioProjects: PortfolioProject[] = [
  {
    ...
    role: '产品定义 / 信息架构 / 前端主链路 / 后台运营 / 数据看板',
    heroLine: 'AI 工具不缺收录，真正难的是把信息变成可判断、可运营、可持续更新的产品。',
    summary:
      '这个项目最直接地展示了我怎么拆需求、怎么取舍功能、怎么把一堆内容收进同一套架子里——而且真正上了线。',
    detailHeadline:
      '导航站做到后来，核心目标变成了帮用户做选型判断，而不只是多堆一页工具列表。',
    coreQuestion:
      'AI 工具一直在冒，但用户真正卡住的地方不是"找不到"，而是"不知道哪个适合我"。这个项目已经从工具目录推进成了工具导航、内容聚合、投稿审核、媒体管理和运营分析一体的系统。',
    primaryValue: {
    ...
    highlights: [
      ...
      '整理出工具、快讯、教程、工作流、专题文章多类内容板块，后台运营入口同步落地。',
      '首页、筛选、详情、搜索、排行榜、投稿审核、媒体库、analytics 后台——关键链路都有可演示版本。',
    ],
    ownedWorkstreams: [
      '产品定位收口：公开站、搜索对比、投稿审核、后台运营，放进同一套信息架构里。',
      '体验梳理 + 前端落地：首页、分类、详情、搜索、排行榜、投稿入口、后台管理等关键页面。',
      '内容分层设计：工具、快讯、教程、工作流、专题文章共同支撑站内判断。',
      '上线前收口：OSS 媒体存储、内容维护流程、analytics 看板和生产部署边界。',
    ],
    deliveryChoices: [
      '主链路定成"发现→判断→对比→跳转"，让整个站更像选型工具而不是资讯聚合。',
      '工具、教程、工作流、快讯之间建了内容网络，站内互相导流，内容也能复用。',
      '后台运营能力保留了，但对外只展示公开效果和抽象职责，不涉及内部运营细节。',
      '数据层从 JSON 演进到 Prisma + PostgreSQL，同时保留本地 JSON 回退，生产部署更稳。',
    ],
    publicArtifacts: [
  ...
  {
    id: 'vestige',
    title: 'Blackborder / 黑域边界',
    subtitle: '结构化跑团引擎驱动的单人 COC 调查游戏',
    timeframe: '2026.01 - 2026.05',
    role: '游戏系统设计 / AI 运行时架构 / 模组内容创作 / 前端与桌面打包',
    heroLine: '玩家说话，规则裁定，状态记账，AI 演出。',
    summary:
      '辨识度最高的一个项目。叙事、规则、内容、运行时产品化——从头到尾连成了一条完整的线。',
    detailHeadline:
      '单人 COC 调查从最初的"AI 守秘人"想法开始，最后推进到了有规则裁定、状态写回、实机界面和桌面包的可玩 Demo。',
    coreQuestion:
      'AI 侦探游戏最怕沦为聊天框：玩家输入自由了，事实、线索和节奏却失控了。《黑域边界》现在走的是自然语言行动、显式 D100 检定、线索写回和 AI 守秘人叙事分层协作的路线。',
    primaryValue: {
    ...
    highlights: [
      '针对 AI 侦探游戏的失控问题，设计了语义裁定、显式检定、状态写回和失败前进四套核心机制。',
      '系统抽象成了"玩家输入 -> 规则裁定 -> 状态记账 -> AI 演出"的结构，关键事实不交给模型随口决定。',
      '原创模组《黑域边界》已收束出决赛 Demo 主路径：委托包签发、酒吧接案、检定、线索板、外勤调查。',
    ],
    ownedWorkstreams: [
      '核心玩法收口：自然语言行动、显式检定、失败前进、线索写回和场景推进共同组成调查闭环。',
      '模块边界拆分：SemanticAdjudicator、TurnPlan、BeatDirector、NarratedTurnExecutor、StateCommitter。',
      '原创模组《黑域边界》：从四幕设定收束到决赛 Demo 的开场、序幕和第一幕完整主路径。',
      '产品化落地：Vue 前端、Python runtime、SSE 流式状态、Electron 解压即用目录包。',
    ],
    deliveryChoices: [
      '自然语言是主入口，但状态、检定、事实提交全走结构化通道——避免纯聊天失控。',
      '检定卡直接展示技能、对象、难度、成功影响和失败前进，让玩家能看懂系统为什么这样判。',
      '委托包签发终端、线索板、持有物品和三栏调查界面共同服务"冷调卷宗"视觉方向。',
      '发布策略从单文件 exe 改成 Windows 解压目录包，换来更快启动和更稳的后端拉起。',
    ],
    publicArtifacts: [
      '委托包签发、语义裁定、显式检定、线索写回和场景切换的实机序列。',
      'Python 运行时架构图，自然语言入口和规则、状态管理之间怎么协作。',
      '决赛 Demo 桌面打包链路，Windows 解压目录包可直接启动。',
      'AI 协作日志与 handoff 文档，记录项目如何从提案推进到可演示版本。',
    ],
    showcaseBoundary:
      '系统结构、玩法机制、实机界面和决赛 Demo 主路径可以公开。完整剧情剧透、Provider 配置、API Key 和私有存档不会放出来。',
    gallery: [
      {
      ...
      {
        src: './resource/img/blackborder-intake-202605.png',
        alt: '黑域边界委托包签发终端截图',
        caption: '委托包签发终端：玩家先确认身份、背景和任务，再进入第一幕现场。',
      },
      {
        src: './resource/img/blackborder-check-request-202605.png',
        alt: '黑域边界检定请求截图',
        caption: '检定请求：系统把技能、对象、难度、成功影响和失败前进都摊开给玩家看。',
      },
      {
        src: './resource/img/blackborder-check-result-202605.png',
        alt: '黑域边界检定结果写回截图',
        caption: '线索写回：检定结果进入守秘人叙事，同时写入线索板和物品状态。',
      },
      {
        src: './resource/img/blackborder-lango-streets-202605.png',
        alt: '黑域边界兰戈区街道截图',
        caption: '兰戈区街道：第一幕从酒吧接案推进到外勤调查，Demo 主路径已经能走通。',
      },
    ],
    diagram: {
      src: './resource/diagrams/vestige-system-architecture.json',
    },
    metrics: ['第一幕完整 Demo', '自然语言 -> 检定 -> 状态写回', 'Electron 桌面目录包', '真实 Provider + 本地测试路径'],
    evidence: ['实机序列截图', '运行时架构设计', 'AI 协作日志', '桌面打包链路'],
